...
const userCards = [
  ...
  },
  {
    ...
    steps: ['Без кода используется текущая карта.', 'Бот создаёт команду для получателя.', 'Команду нужно переслать новому владельцу.'],
    warning: 'Передать можно только карту, владельцем которой является текущий пользователь.',
  ...
  },
  {
    ...
    steps: ['Получатель отправляет команду с кодом.', 'Бот проверяет срок действия и статус кода.', 'Карта становится текущей картой получателя.'],
    warning: 'Код должен быть действующим, неиспользованным и не принадлежать тому же пользователю.',
    footer: 'Команда видна в глобальном меню Telegram.',
  },
  {

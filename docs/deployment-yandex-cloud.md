...
Production runs as separate Yandex Serverless Containers for the API and Telegram webhook bot. Release images are stored in Yandex Container Registry, PostgreSQL runs in Yandex Managed Service for PostgreSQL, and application secrets are stored in Yandex Lockbox.

This document reflects the current `.github/workflows/release.yml`.
...
- builds and pushes API, bot webhook, and migrations images;
- runs migrations from the GitHub Actions runner using the migrations image;
- deploys API and bot Serverless Container revisions;
...
`TELEGRAM_WEBHOOK_SECRET` is generated by us, not by Telegram or Yandex Cloud:
...
The bot webhook container receives:
...
The release workflow reads the values needed by migrations and Telegram webhook registration:
...
The workflow masks each value and writes it to `$GITHUB_ENV` for later steps. Runtime API and bot revisions receive secrets through `revision-secrets`.

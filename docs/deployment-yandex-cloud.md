...
- uploads the static QR Mini App HTML to Yandex Object Storage;
...
- Container Registry.
- Object Storage bucket for the static QR Mini App HTML.
...
`WEB_APP_URL` must point to the static QR Mini App HTML in Object Storage:

```text
https://storage.yandexcloud.net/<qr_mini_app_bucket>/qr.html
```

`TELEGRAM_WEBHOOK_SECRET` is only required by the old manual webhook workflow. It is generated by us, not by Telegram or Yandex Cloud:
...
## QR Mini App Object Storage

Create a dedicated small public bucket before creating the Lockbox secret, because `WEB_APP_URL` must point to the final object URL:
...
The CI service account needs permission to upload the object during release, for example `storage.editor` scoped to the folder or to this bucket.

Store this bucket name in GitHub Actions:

```bash
gh variable set YC_QR_MINI_APP_BUCKET \
  --env production \
  --body ion-gift-card-qr-mini-app-<suffix>
```

Set the Lockbox `WEB_APP_URL` value to:

```text
https://storage.yandexcloud.net/ion-gift-card-qr-mini-app-<suffix>/qr.html
```

The release workflow uploads `src/api/qr-mini-app.html` to `s3://$YC_QR_MINI_APP_BUCKET/qr.html` and then verifies that Lockbox `WEB_APP_URL` matches that Object Storage URL.
...
5. Uploads `src/api/qr-mini-app.html` to `s3://$YC_QR_MINI_APP_BUCKET/qr.html` with public read.
6. Verifies that Lockbox `WEB_APP_URL` points to `https://storage.yandexcloud.net/$YC_QR_MINI_APP_BUCKET/qr.html`.
